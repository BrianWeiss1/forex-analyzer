...
class Trade(_HTTPManager):
    ...
    def change_leverage(self, symbol: str, positionSide: PositionSide, leverage: int, recvWindow: int | None = None) -> dict[str, Any]:
        """
        ...
        endpoint = "/openApi/swap/v2/trade/leverage"
        payload = {"symbol": symbol, "side": positionSide.value, "leverage": leverage} if recvWindow is None else {"symbol": symbol, "side": positionSide.value, "leverage": leverage, "recvWindow": recvWindow}
    ...
    def change_isolated_margin(self, symbol: str, amount: float, type: int, positionSide: PositionSide = PositionSide.LONG, recvWindow: int | None = None) -> dict[str, Any]:
        """
        ...
        payload = {"symbol": symbol, "amount": amount, "type": type, "positionSide": positionSide.value} if recvWindow is None else {"symbol": symbol, "amount": amount, "type": type, "positionSide": positionSide.value, "recvWindow": recvWindow}
