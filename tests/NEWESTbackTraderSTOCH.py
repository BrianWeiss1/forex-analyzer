...
def getData():
    ...
    dfIndex = df.index[0]
    return df
def SMA(data, days=100):
    DaytoMin = days*24*2
    return data['close'].rolling(window=DaytoMin).mean()
run()
class OpeningRangeBreakout(bt.Strategy):
    ...
    def next(self):
        ...
        sma_30 = self.params.sma_30
        # data['Market Condition'] = 'Sideways'
# data.loc[data['Close'] > data['SMA_30'], 'Market Condition'] = 'Bullish'
# data.loc[data['Close'] < data['SMA_30'], 'Market Condition'] = 'Bearish'
        ...
        #     #     self.close()  # Close the long position if take-profit is triggered
        stage = ""
        if not np.isnan(sma_30[i]):
            if self.data.close[0] > sma_30[i]:
                stage = "BULLISH"
            elif self.data.close[0] < sma_30[i]:
                stage = "BEARISH"
            else:
                stage == None
            
        
        if (not np.isnan(rsiK[i]) and not np.isnan(rsiD[i])):  
            ...
            if self.longCount == 1 and stage == 'BEARISH':
            ...
            elif self.shortCount == 1 and stage == 'BULLISH':
                self.size = ((self.betPercent * self.broker.cash)/ self.data.close[0])
                # if self.position:
                #     self.close() #size=self.position.size
...
try:
    for j in range(1, 150, 1):
        ...
        NP_sma_30 = np.array(sma_30)
        data = bt.feeds.PandasData(dataname=df)
        cerebro.adddata(data)
        cerebro.addstrategy(OpeningRangeBreakout, rsiK=stochRSIk, rsiD=stochRSId, sma_30=NP_sma_30)
